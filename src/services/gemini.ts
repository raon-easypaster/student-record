...
export async function analyzeUniversityMatch(
  ...
  try {
    ...
    const prompt = `
      ...
      [분석 가이드라인 - 일반적이고 추상적인 제안 금지]
      1. 강점 및 약점 도출 시, 학생의 실제 과목명, 등급 수치, 동아리 탐구 명칭 등 구체적인 텍스트 데이터를 명확히 인용할 것.
      2. 보완 제안 시, "독서를 해라", "성적을 올려라"와 같은 뻔한 조언은 절대 금지하며, 학생의 현재 학년 및 기재 상태에 기반하여 "수학 성적 X등급 대비 요강 기준이 높으므로 다음 학기 세특에 어떤 개념(구체적 공학 키워드)을 도서 [명칭]과 엮어 작성하라" 등 실행 가능한 타겟 과제를 지목할 것.
      3. 종합 분석 텍스트('analysis_text') 맨 끝에 반드시 다음 문구를 한 문단 띄운 후 개행하여 글자 토씨 하나 틀리지 않고 그대로 덧붙여 포함시킬 것:
...
export async function generateFinalAdmissionReport(
  ...
  try {
    ...
    const prompt = `
      ...
      [결과 요구 사항]
      1. recommended_activity_ids: 학생의 누적 활동들 중 전공 적합성이 가장 강력한 3가지 활동의 실제 ID 목록을 추천해줘.
      2. essay_materials: 자소서 문항별(학업, 진로, 공동체)로 가장 부합하는 학생의 실제 활동을 지칭하고, 그 활동에서 어떤 전공적 소양과 갈등 극복 과정을 녹여 써야 매력적인지 맞춤 가이드를 한글로 제공해줘.
      3. interview_preps: 실제 학생의 생기부 기재 내용에 근거한 모의 면접 예상 질문 2가지를 뽑고, 관련 활동명과 면접 답변을 구상할 때 들어가야 할 논리 구조 팁을 상세히 작성해줘.
