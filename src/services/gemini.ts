...
export async function analyzeUniversityMatch(
  ...
  try {
    ...
    const prompt = `
      ...
      1. 강점 및 약점 도출 시, 학생의 실제 과목명, 등급 수치, 동아리 탐구 명칭 등 제공된 JSON 데이터에 있는 구체적인 텍스트만 명확히 인용할 것.
      ...
      4. 종합 분석 텍스트('analysis_text') 맨 끝에 반드시 다음 문구를 한 문단 띄운 후 개행하여 그대로 덧붙여 포함시킬 것:
